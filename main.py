...
class Dinosaur:
    ...
    def __init__(self): # Constructor
        self.duck_img = NUNDUK
        self.run_img = LARI
        self.jump_img = LOMPAT
    ...
    while run:
        ...
        if len(obstacles) == 0:
            if random.randint(0, 2) == 0:
                obstacles.append(SmallCactus(BATU))
            elif random.randint(0, 2) == 1:
                obstacles.append(LargeCactus(KAKTUS))
...
def menu(death_count):
    ...
    while run:
        ...
        SCREEN.blit(text, textRect)
        SCREEN.blit(LARI[0], (SCREEN_WIDTH // 2 - 20, SCREEN_HEIGHT // 2 - 140))
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
